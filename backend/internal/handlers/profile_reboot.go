...
func fetchProfileDetails(r *http.Request, login string) map[string]any {
	...
	defer cancel()
	payload, _ := json.Marshal(map[string]any{"query": `query ProfileDetails($login: String!) { user(where: {login: {_eq: $login}}, limit: 1) { auditRatio attrs } group_user(where:{userLogin:{_eq:$login},accepted:{_eq:true}},order_by:{group:{updatedAt:desc}}) { group { id status path updatedAt object { name type } progresses(where:{userLogin:{_eq:$login}},order_by:{updatedAt:desc},limit:1) { grade isDone } } } progress(where:{userLogin:{_eq:$login},path:{_like:"/bahrain/bh-module/%"}},order_by:{updatedAt:desc}) { id path updatedAt grade isDone object { name type } } }`, "variables": map[string]string{"login": login}})
	...
	if res.StatusCode != http.StatusOK || json.NewDecoder(res.Body).Decode(&result) != nil || len(result.Data.User) == 0 {
		for _, issue := range result.Errors {
			log.Printf("profile project query: %s", issue.Message)
		}
		return nil
	}
	// Include individual project progress as well as accepted group memberships.
	for _, p := range result.Data.Piscines {
		path, _ := p["path"].(string)
		parts := strings.Split(strings.TrimRight(path, "/"), "/")
		isPiscine := strings.HasPrefix(parts[len(parts)-1], "piscine-")
		object, _ := p["object"].(map[string]any)
		if !isPiscine && (object["type"] != "project" || strings.Contains(path, "/piscine-")) {
			continue
		}
		status := "working"
		...
		result.Data.Projects = append(result.Data.Projects, map[string]any{"group": map[string]any{
			"id": p["id"], "status": status, "path": path, "updatedAt": p["updatedAt"], "object": p["object"], "isPiscine": isPiscine,
