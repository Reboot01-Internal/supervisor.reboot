      ...
      >
        <div className="flex flex-col gap-3">
          ...
          {isAdmin ? (
            ...
            <>
              {role === "supervisor" && <SidebarItem currentActive={active} onNavigate={onNavigate} id="reports" label="Reports" to="/admin/reports" icon={<svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" aria-hidden="true"><path d="M5 20V10m7 10V4m7 16v-7" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>} />}
        ...
        <SidebarAccount name={footerName} photo={avatarUrl} role={isAdmin ? "Admin" : role || "Workspace"} dark={darkMode} onTheme={onToggleTheme} onProfile={() => {nav("/profile");onNavigate?.();}} onLogout={() => {logout();onNavigate?.();}} />
      </div>
    </aside>
  );
}
