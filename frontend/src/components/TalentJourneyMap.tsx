...
function loadJourney(force:boolean) {
 ...
 journeyCache=entry;
 entry.pending=(async()=>{
  const talents:Talent[]=await apiFetch('/admin/users?include_inactive=1&role=student');
  const profiles:Record<number,Profile|null>={};
  const photos=fetchRebootAvatars(talents.map(t=>t.nickname).filter(Boolean)).catch(()=>({}));
  let cursor=0;
  await Promise.all(Array.from({length:3},async()=>{while(cursor<talents.length){const t=talents[cursor++];try{profiles[t.id]=await apiFetch(`/admin/profile/summary?user_id=${t.id}&reboot_details=1`);}catch{profiles[t.id]=null;}}}));
...
export default function TalentJourneyMap({revision}:{revision:number}) {
 ...
 useEffect(()=>{
  ...
  loadJourney(force).then(snapshot=>{
   if(!active)return;
   setTalents(snapshot.talents);setProfiles(snapshot.profiles);setAvatars(snapshot.avatars);
  }).catch(()=>{if(active)setError('Could not load the talent journey. Use Refresh to try again.');})
 ...
 <div className="talent-map-status" role="status">{error||`${visible.length} talents shown${loading?` · Loading talent profiles…`:''}`}</div>
