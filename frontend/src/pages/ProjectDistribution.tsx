...
type User = {cohort?:string;assigned_boards?:string[]};
...
export default function ProjectDistribution({boards,users,avatars,loading,error}:{boards:Board[];users:User[];avatars:Record<string,string>;loading:boolean;error:string}) {
 ...
 const cohorts=[...new Set(users.map(u=>u.cohort).filter((c):c is string=>Boolean(c)))].sort();
 const cohortBoards=cohort==='all'?boards:boards.filter(b=>users.some(u=>u.cohort===cohort&&u.assigned_boards?.includes(b.name)));
