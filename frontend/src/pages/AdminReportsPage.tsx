...
export default function AdminReportsPage(){
 ...
 useEffect(()=>{let alive=true;setLoading(true);setError('');(async()=>{try{
  const [b,s]=await Promise.all([apiFetch('/admin/all-boards'),isAdmin?apiFetch('/admin/supervisors'):Promise.resolve([])]);
  if(!alive)return;setBoards(b);setPeople(s);
  if(!isAdmin){const p=await apiFetch("/admin/profile/summary");if(alive)setAssigned((p.supervisor?.assigned_students||[]).map((u:any)=>({...u,role:"student"})));}
 ...
 }catch{if(alive)setError('Could not load reports. Try refreshing.');}finally{if(alive)setLoading(false);}})();return()=>{alive=false;};},[revision,isAdmin]);
 useEffect(()=>{if(!isAdmin)return;let alive=true;setActivity(null);setActivityError('');apiFetch(`/admin/dashboard/supervisor-activity?week_offset=${week}`).then(a=>{if(alive)setActivity(a);}).catch(()=>{if(alive)setActivityError('Activity unavailable');});return()=>{alive=false;};},[week,revision,isAdmin]);
